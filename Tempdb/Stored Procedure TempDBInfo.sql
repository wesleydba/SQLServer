--- fonte : https://github.com/aleksey-vitsko/Database-Administrator-Tools/blob/master/TempDB%20-%20TempDBInfo.sql
use master
go
...
*************************************************************************************************************************/
    ...
    -- data files
    insert into #TempDB_Files_SpaceUsage
        (tFileID, tTotal_MB, tAllocated_MB, tUnallocated_MB, tVersionStore_MB, tUserObject_MB, tInternalObject_MB, tMixedExtent_MB, tModifiedExtent_MB)
    select
        [file_id],
    ...
    -- log file
    insert into #TempDB_Files_SpaceUsage
        (tFileID, tName, tPhysical_Name, tState_Desc, tTotal_MB)
    select
        [file_id],
        [name],
        physical_name,
...
from #TempDB_Files_SpaceUsage
    ...
    set @DataFile_SpaceUsed_Percent = (select (sum(tAllocated_MB) / sum(tTotal_MB)) * 100
    from #TempDB_Files_SpaceUsage
    where tType_Desc = 'ROWS')


    -- allocated / unallocated for data file(s)
    set @DataFile_Allocated_MB = (select sum(tAllocated_MB)
    from #TempDB_Files_SpaceUsage
    where tType_Desc = 'ROWS')

    set @DataFile_Unallocated_MB = (select sum(tUnallocated_MB)
    from #TempDB_Files_SpaceUsage
    where tType_Desc = 'ROWS')
    ...
    insert into #DBCCSQLPerfLogSpace
        ([DB_Name], LogSize, LogSpaceUsedPct, [Status])
    exec ('dbcc sqlperf (logspace)')



    set @Log_SpaceUsed_Percent = (select LogSpaceUsedPct
    from #DBCCSQLPerfLogSpace
    where [DB_Name] = 'tempDB')
    set @Log_SpaceUsed_MB = (select (LogSize * LogSpaceUsedPct) / 100
    from #DBCCSQLPerfLogSpace
    where [DB_Name] = 'tempDB')
...
*/
    ...
    insert into #TempDB_Info
        (Property, [Value])
    values
        ('Total TempDB size ', cast(@TempDB_Total_MB as varchar) + ' Megabytes ( ' + cast(@TempDB_Total_MB / 1024 as varchar) + ' GB )'),
    ...
    insert into #TempDB_Sessions
        (Session_ID, Login_Name, User_Alloc_MB, User_Dealloc_MB, Internal_Alloc_MB, Internal_Dealloc_MB, Current_MB)
    select
        ssu.session_id,
        s.login_name,
    ...
    insert into #TempDB_Tasks
        (Session_ID, Login_Name, Task_Address, User_Alloc_MB, User_Dealloc_MB, Internal_Alloc_MB, Internal_Dealloc_MB, Current_MB)
    select
        tsu.session_id,
        s.login_name,
        task_address,
